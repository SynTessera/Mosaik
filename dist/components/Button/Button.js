...
export const Button = ({ className, children, onClick, tooltip, allowDisabledClick, onDisabledClick, id, tooltipPlacement, as = "button", size = Sizes.auto, fit = [FitX.auto], colorMode, ...rest }) => {
    ...
    return (<>
      <Cmp id={id} title={rest.title} onClick={(e) => {
        ...
        })} {...rest} disabled={!allowDisabledClick && disabled}>
        {children}
        <div className="absolute w-full h-full bg-black/0 hover:bg-black/10  z-20 group-disabled:hidden left-0"/>
      </Cmp>
      {ReactDOM.createPortal(<Tooltip anchorSelect={"#" + id} place={tooltipPlacement} className="z-[10000]">
          {tooltip}
        </Tooltip>, document.querySelector("dialog[open]") || document.body)}
    </>);
};
export const IconButton = ({ className, iconClsn = "", round, tooltip, icon, allowDisabledClick, onDisabledClick, ...rest }) => {
    return (<Button className={clsx("IconButton", " flex items-center justify-center  disabled:text-gray-400 gap-1 ", className, {
            "text-gray-400": rest.disabled,
            "!rounded-[200px]": round,
        })} tooltip={tooltip} {...rest} disabled={rest.disabled} allowDisabledClick={allowDisabledClick} onDisabledClick={onDisabledClick}>
      {icon && (<Icon className={clsx(
            // "p-[4px]",
            "pointer-events-none cursor-grab h-full w-full", iconClsn)} icon={icon}/>)}
      {rest.children}
    </Button>);
};
export const LinkButton = ({ as = "a", active, to, ...rest }) => {
    ...
    return (<LinkIconButton as={as} to={to} {...rest} className={clsx("!border-0 !pr-2 items-center", rest.className, {
...
export const MenuButton = ({ className, tooltip, icon, ...rest }) => {
    ...
    return (<div>
      <div ref={ref}>
        ...
        }} className={clsx("relative", className)} tooltip={tooltip} {...rest}>
          {icon && <Icon icon={icon}/>}
        </ButtonButton>
      </div>
      {ReactDOM.createPortal(<div className={clsx("absolute bottom-[100%] z-[4000] w-[100px] h-[100px]", {
                hidden: !menuOpen,
            })} style={{
                left: x - 48,
                top: y,
